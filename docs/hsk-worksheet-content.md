...
The exported `MAX_TOTAL_IMAGE_CHARS` and `imageDataSize(workbook)` support upload-budget checks; `validateWorkbook`, `mergeWorkbook`, and `saveWorkbook` enforce the limit with `WorkbookSizeError`. The browser keeps a local draft in IndexedDB. Edits automatically publish a shared copy to private Vercel Blob storage after a short pause, without login; anyone with the website link can edit it. Save to website provides an explicit retry. Only a server acknowledgement marks the workbook shared, and edits during an upload trigger another save. Local-only `cloudRevision` and `cloudDirty` metadata preserve unpublished drafts and detect stale saves, and are stripped from portable backups. Browser data can be cleared, so keep exported backups of completed work.

The shared copy loads independently of browser storage. Private-browser storage failures show a warning but do not block viewing published pictures and keys. Returning to a clean page checks fresh metadata; unchanged revisions avoid re-downloading all pictures. Unpublished drafts are never silently replaced. An older browser-only draft with no recorded website revision requires explicit review before sharing.
